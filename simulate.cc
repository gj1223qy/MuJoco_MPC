...
void UpdateInfoText(mj::Simulate* sim,
  ...
  // 计算求解器误差
  int island = 0;  // 仅第一个岛
  mjtNum solver_error = 0;
  
  if (d->solver_niter[island] > 0) {
    int index = mjMIN(d->solver_niter[island] - 1, mjNSOLVER - 1);
    const mjSolverStat* stat = d->solver + island * mjNSOLVER + index;
    solver_error = mju_min(stat->improvement, stat->gradient);
    if (solver_error == 0) {
      solver_error = mju_max(stat->improvement, stat->gradient);
    }
  }
  solver_error = mju_log10(mju_max(mjMINVAL, solver_error));
  ...
  if (best_trajectory) {
    int num_params = sim->agent->ActivePlanner().NumParameters();
    ...
    mju::sprintf_arr(content, 
        ...
        num_params,
        d->time, 
        d->maxuse_arena / static_cast<double>(d->narena),
  ...
  if (mjENABLED(mjENBL_ENERGY)) {
    char energy_str[32];
    mju::sprintf_arr(energy_str, "\n%.3f", d->energy[0] + d->energy[1]);
    mju::strcat_arr(content, energy_str);
  ...
  if (mjENABLED(mjENBL_ISLAND)) {
    char island_str[16];
    mju::sprintf_arr(island_str, "\n%d", d->nisland);
    mju::strcat_arr(content, island_str);
    mju::strcat_arr(title, "\nIslands");
  }
}
